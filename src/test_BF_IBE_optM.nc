...
module test_BF_IBE_optM{
    uses interface Boot;
    uses interface Leds;
    uses interface BF_IBE_opt;
...
implementation{
    ...
    for(i=0;i<10;i++){
        ...
        call BF_IBE_opt.setup(&generator, &mk, &pk);
        end = call Counter.get();
        tmsg->t1 += end - start;
        
        start = call Counter.get();
        call BF_IBE_opt.keyextract(ID, sizeof(ID), &mk, &dID);
        end = call Counter.get();
        tmsg->t2 += end - start;
        
        start = call Counter.get();
        call BF_IBE_opt.encrypt(&generator, &pk, msg, sizeof(msg), ID, sizeof(ID), &C0, C1);
        end = call Counter.get();
        tmsg->t3 += end - start;
        
        start = call Counter.get();
        call BF_IBE_opt.decrypt(&dID, &C0, C1, sizeof(C1), dmsg);
        end = call Counter.get();
        tmsg->t4 += end - start;
    }
